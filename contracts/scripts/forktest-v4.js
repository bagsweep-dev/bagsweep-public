...
//
// ⚠ CANNOT run until $REAP has graduated to a real V4 pool AND you can fork RH Chain.
...
//        UNIVERSAL_ROUTER, PERMIT2, USDG_ADDR, REAP_ADDR, POOL_FEE, POOL_TICK_SPACING,
...
// SweepBuyback buys back + burns $REAP to dEaD through the adapter. Run it FIRST inside
...
async function main() {
  ...
  const USDG = need("USDG_ADDR"), REAP = need("REAP_ADDR");
  ...
  const reap = await ethers.getContractAt("IERC20", REAP);
  ...
  await (await adapter.setPoolKey(USDG, REAP, FEE, TICK, HOOK)).wait();
  console.log(`adapter ${await adapter.getAddress()} · PoolKey(USDG,REAP,${FEE},${TICK},${HOOK}) set`);

  // 2. Direct adapter swap from a contract-less caller path is fine; the real question is
  //    a contract-INITIATED swap works and the plan bytes are right. Route USDG->REAP.
  await (await usdg.approve(await adapter.getAddress(), CANARY)).wait();
  const before = await reap.balanceOf(dep.address);
  const dl = (await ethers.provider.getBlock("latest")).timestamp + 600;
  await (await adapter.swapExactTokensForTokens(CANARY, 1n, [USDG, REAP], dep.address, dl)).wait();
  const got = (await reap.balanceOf(dep.address)) - before;
  if (got <= 0n) throw new Error("direct swap returned 0 REAP — check PoolKey / plan bytes");
  const p2Allowance = await usdg.allowance(await adapter.getAddress(), PERMIT2);
  console.log(`✓ direct swap: ${CANARY} USDG -> ${got} REAP · permit2 allowance now ${p2Allowance} (want 0)`);
  ...
  const deadBefore = await reap.balanceOf(DEAD);
  ...
  const swapData = iface.encodeFunctionData("swapExactTokensForTokens", [CANARY, 1n, [USDG, REAP], await buyback.getAddress(), dl2]);
  await (await buyback.buybackAndBurn(CANARY, 1n, await adapter.getAddress(), swapData)).wait();
  const burned = (await reap.balanceOf(DEAD)) - deadBefore;
  if (burned <= 0n) throw new Error("buyback burned 0 REAP");
  console.log(`✓ buyback burned ${burned} REAP to dEaD through the adapter`);
