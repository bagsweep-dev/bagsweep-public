...
//
// ⚠ CANNOT run until $SWEPT has graduated to a real V4 pool AND you can fork RH Chain.
...
// SweepBuyback buys back + burns $SWEPT to dEaD through the adapter. Run it FIRST inside
...
async function main() {
  ...
  const USDG = need("USDG_ADDR"), SWEPT = need("SWEEP_ADDR");
  ...
  const sweep = await ethers.getContractAt("IERC20", SWEPT);
  ...
  await (await adapter.setPoolKey(USDG, SWEPT, FEE, TICK, HOOK)).wait();
  console.log(`adapter ${await adapter.getAddress()} · PoolKey(USDG,SWEPT,${FEE},${TICK},${HOOK}) set`);

  // 2. Direct adapter swap from a contract-less caller path is fine; the real question is
  //    a contract-INITIATED swap works and the plan bytes are right. Route USDG->SWEPT.
  ...
  await (await adapter.swapExactTokensForTokens(CANARY, 1n, [USDG, SWEPT], dep.address, dl)).wait();
  const got = (await sweep.balanceOf(dep.address)) - before;
  if (got <= 0n) throw new Error("direct swap returned 0 SWEPT — check PoolKey / plan bytes");
  const p2Allowance = await usdg.allowance(await adapter.getAddress(), PERMIT2);
  console.log(`✓ direct swap: ${CANARY} USDG -> ${got} SWEPT · permit2 allowance now ${p2Allowance} (want 0)`);
  ...
  const swapData = iface.encodeFunctionData("swapExactTokensForTokens", [CANARY, 1n, [USDG, SWEPT], await buyback.getAddress(), dl2]);
  ...
  if (burned <= 0n) throw new Error("buyback burned 0 SWEPT");
  console.log(`✓ buyback burned ${burned} SWEPT to dEaD through the adapter`);
