...
async function main() {
  ...
  // Buyback-and-burn job. OFF until $SWEPT launches (BUYBACK_ENABLED=1). Cooldown-gated
  ...
  // $SWEPT gate price refresh (dollar-peg). Off unless the gate is on and not forced fixed.
  if (config.gate.enabled && config.gate.mode !== "fixed") {
...
async function runEvalCycle() {
  ...
  for (const plan of plans) {
    try {
      ...
      if (!userOp) {
        // Demand gate: owner not $SWEPT-entitled. A skip, not a failure — the account
        // keeps its ungated self-exit (ownerExecute); the keeper just doesn't automate it.
        stats.sweepsSkippedGate++;
        console.log(`[sweep] Skipped ${plan.account}: owner not $SWEPT-entitled (free tier keeps the self-serve exit)`);
        continue;
      }
