...
// Per-host send pacer: guarantees a minimum gap between successive requests to the same host,
// so concurrent lookups can't burst past a provider's per-IP rate limit. RH's public RPC 429s
// on bursts and sends no Retry-After, so we shape the send rate instead of only reacting to it.
// Chains one promise per host; each caller awaits its slot, then stamps its send time.
const _hostLast = new Map(); // host -> Promise<lastSendMs>
export function pace(url, minGapMs) {
  if (!minGapMs) return Promise.resolve();
  let host; try { host = new URL(url).host; } catch { host = url; }
  const prev = _hostLast.get(host) || Promise.resolve(0);
  const next = prev.then(async (last) => {
    const wait = minGapMs - (Date.now() - last);
    if (wait > 0) await sleep(wait);
    return Date.now();
  });
  _hostLast.set(host, next);
  return next;
}
...
// ---- EVM JSON-RPC ----
// RH's public RPC rate-limits per IP and 429s on bursts (concurrent lookups add up), so every
// send is paced by a per-host min gap and the retry rides out a 429 that still slips through.
const RPC_MIN_GAP_MS = Number(process.env.RPC_MIN_GAP_MS || 200);
const RPC_RETRY = { tries: 5, baseMs: 500, retryOn: (e) => e.status === 429 || e.status >= 500 || !e.status };

export async function evmRpc(url, method, params) {
  const body = await withRetry(async () => {
    await pace(url, RPC_MIN_GAP_MS);
    return jfetch(url, {
      method: "POST",
