...
export async function rhPortfolio(address) {
  ...
  const notes = [];
  // Overlap the independent fetches: the slow paginated history, the balances (its own
  // Blockscout call sequence), and the stock/native lookups all run at once, so only the
  // token-price lookup waits on the token set. Cuts wall-clock vs the old strictly
  // sequential order. (The cursor-paginated history itself is inherently serial: each page
  // needs the previous page's next_page_params, so it cannot be split further.)
  const [logsResult, balancesSettled, stocks, natives] = await Promise.all([
    walletLogs(wallet),
    blockscoutBalances(wallet).then((v) => ({ ok: true, v })).catch((e) => ({ ok: false, e })),
    stockTokens(),
    nativePrices(),
  ]);

  const { logs, truncated, tokenInfo, blockTime } = logsResult;
  if (truncated) notes.push(`History truncated to the newest ${logs.length} transfers.`);
  const txCount = new Set(logs.map((l) => l.tx)).size;

  // Balances from the indexer; fall back to the RPC only if the indexer call failed (the
  // fallback needs the token list from history, which is ready by now).
  let nativeWei, balMap, metaMap;
  if (balancesSettled.ok) {
    ({ nativeWei, balMap, metaMap } = balancesSettled.v);
  } else {
  ...
  const ds = await dexPrices("robinhood", tokens);
