...
- Fee/buyback (optional, for §4 S4): deploy a mock `$SWEPT`, deploy `SweepBuyback`, `buyback.setSweepToken($SWEPT)`, `buyback.setSanctionedRouter(sweepRouter, true)`, `executor.setTreasury(buyback)`, `executor.setFeeBps(50)`.
...
| S4 | Fee on (feeBps=50, treasury=buyback) | `FeeCollected` emitted, 0.5% of proceeds skimmed to `SweepBuyback`; then `buyback.buybackAndBurn(...)` burns $SWEPT, `BuybackBurned` emitted |
...
| N9 | **Buyback no-exit:** `buyback.rescue(USDG, …)` and `rescue($SWEPT, …)` | both revert `CannotRescueProtocolAsset`; USDG only leaves via `buybackAndBurn` |
